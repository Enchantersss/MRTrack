...
class DetMOTDetection:
    ...
    def __getitem__(self, idx):
        if idx < len(self.indices):
            ...
            indices = self.sample_indices(vid, f_index)  # e.g. [t0, t1, t2, ...]
            images, targets = self.pre_continuous_frames(vid, indices)

            dts = [float(indices[i] - indices[i-1]) for i in range(1, len(indices))]
        else:
            images, targets = self.load_crowd(idx - len(self.indices))
            dts = [1.0 for _ in range(self.num_frames_per_batch - 1)]
